"""
CLIP Classifier for Business-Relevant Object Detection

Призначення:
- Використовує pretrained CLIP модель для zero-shot classification
- Класифікує зображення як релевантні/нерелевантні для бізнес-категорій
- V3.0: Scene Classification approach з парами positive/negative промптів
- Працює з датасетом data/stage_1/ (4 категорії)
- Зберігає результати у results/stage_1/results_clip.json

Структура:
- CLIPClassifier: основний клас для zero-shot classification
- classify_image(): обчислює схожість image-text
- is_relevant(): визначає релевантність порівнюючи positive vs negative
...
class CLIPClassifier:
    def __init__(self, model_name='openai/clip-vit-base-patch32', model_path='weights/clip'):
        ...
        self.processor = CLIPProcessor.from_pretrained(model_name, cache_dir=str(model_dir))
